...
protocol ProductCommentViewControllerDelegate: AnyObject {
...
class ProductCommentViewController: BaseViewController {
    ...
    fileprivate(set) var replyComment: Comment?
    ...
    lazy var replyCommentView: ReplyCommentView = {
    ...
    override func viewDidLoad() {
        ...
        self.replyCommentView.isHidden = true
    ...
    override func touchUpInLeftBarButtonItem() {
        if self.isCheck == true {
            self.navigationController?.popToViewControllerType(NotificationViewController.self,
                                                               completion: nil)
        } else {
    ...
    func configData(productId: Int?, replyComment: Comment? = nil) {
        ...
        self.getCommentsAPI(productId: productId ?? 0)
    }
    ...
    private func postComment(message: String?) {
        ...
        newComment.userAvatar   = currentUser.avatar
        newComment.fullName     = currentUser.name
        ...
        APIService.request(endPoint: endPoint, onSuccess: { [weak self] (apiResponse) in
            ...
            self.chatInputView.sendButton.isUserInteractionEnabled = true
            if let replyComment = self.replyComment {
                ...
                if let index = self.comments.firstIndex(where: { $0.id == parentId }) {
                    self.comments[index].commentChild.insert(newComment,
                                                             at: self.comments[index].commentChild.count)
                }
            } else {
    ...
    func getCommentsAPI(productId: Int) {
        let endPoint = ProductEndPoint.getCommentProduct(params: ["productId": productId])
        APIService.request(endPoint: endPoint) { [weak self] apiResponse in
            guard let self = self else { return }
            self.hideLoading()
            self.comments = apiResponse.toArray([Comment.self])
            self.configData(comments: self.comments)
        } onFailure: { error in
...
extension ProductCommentViewController: DetailCommentCollectionViewCellDelegate {
    func didSelectLikeComment(_ comment: Comment) {
        
    }
    
    func didSelectReplyComment(_ comment: Comment) {
        self.replyCommentView.isHidden = false
        chatInputView.beginEditing()
...
extension ProductCommentViewController: ChatInputViewDelegate {
    func didSelectSendMessage(view: ChatInputView, message: String?) {
        view.sendButton.isUserInteractionEnabled = false
