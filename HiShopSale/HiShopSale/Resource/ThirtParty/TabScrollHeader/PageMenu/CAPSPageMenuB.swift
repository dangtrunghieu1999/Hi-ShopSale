...
@objc public protocol CAPSPageMenuDelegateB {
...
open class CAPSPageMenuB: UIViewController {

    //MARK: - Configuration
    var configuration = CAPSPageMenuConfigurationB()
    ...
    var lastScrollDirection : CAPSPageMenuScrollDirectionB = .other
    var startingPageForScroll : Int = 0
    var didTapMenuItemToScroll : Bool = false

    var pagesAddedDictionary : [Int : Int] = [:]

    open weak var delegate : CAPSPageMenuDelegateB?

    var tapTimer : Timer?

    enum CAPSPageMenuScrollDirectionB : Int {
        case left
        case right
        case other
    }
    ...
    public convenience init(viewControllers: [UIViewController], frame: CGRect, pageMenuOptions: [CAPSPageMenuOptionB]?) {
    ...
    */
    public init(viewControllers: [UIViewController], frame: CGRect, configuration: CAPSPageMenuConfigurationB) {
        super.init(nibName: nil, bundle: nil)
    ...
    /**
     ...
     - parameter configuration: A configuration instance for page menu
     */
    public init(viewControllers: [UIViewController], in controller: UIViewController, with configuration: CAPSPageMenuConfigurationB, usingStoryboards: Bool = false) {
        super.init(nibName: nil, bundle: nil)
        self.configuration = configuration
        controllerArray = viewControllers
        ...
        if pageIndex >= 0 && pageIndex < controllerArray.count {
            UIView.animate(withDuration: 0.15, animations: { () -> Void in
                ...
                if self.menuItems.count > 0 {
                    ...
                    self.menuItems[self.lastPageIndex].iconImageView.tintColor = self.configuration.unselectedMenuItemLabelColor
                    self.menuItems[self.currentPageIndex].titleLabel.textColor = self.configuration.selectedMenuItemLabelColor
                    self.menuItems[self.currentPageIndex].iconImageView.tintColor = self.configuration.selectedMenuItemLabelColor
                }
            })
        }
    }
    ...
    override open func viewDidLayoutSubviews() {
        ...
        if (oldCurrentOrientationIsPortrait && UIDevice.current.orientation.isLandscape) || (!oldCurrentOrientationIsPortrait && (UIDevice.current.orientation.isPortrait || UIDevice.current.orientation.isFlat)) {
            ...
            if configuration.useMenuLikeSegmentedControl {
                ...
                for item : MenuItemViewB in menuItems as [MenuItemViewB] {
